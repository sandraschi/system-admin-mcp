...
4. Start the server:
   ...
   uv run python -m system_admin_mcp.server

   # HTTP mode (for web dashboard)
   uv run uvicorn system_admin_mcp.server:app --port 10861
   ```
5. Open `http://localhost:10861` or the frontend URL.
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/system-admin-mcp/issues) |
