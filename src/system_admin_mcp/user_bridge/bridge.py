...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.expandvars(r"%LOCALAPPDATA%\SystemAdminMCP\bridge.log")),
...
class UserBridge:
    ...
    def _check_disabled_state(self) -> None:
        ...
        if self._disabled:
            logger.warning("Bridge is disabled via marker file. To re-enable, run: "
    ...
    def _check_service_status(self) -> None:
        ...
        try:
            ...
            logger.debug(f"Service status - installed: {self.service_installed}, "
    ...
    def install_service(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.error(
                    f"Failed to install service: {result.stderr}"
                )
                return False
    ...
    def _send_request(self, action: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if not self.service_installed:
            return {"status": "error", "message": "System Admin MCP service is not installed"}
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        if bridge.service_running:
            ...
            for vol in volumes:
                print(f"- {vol.get('name')}: {vol.get('label', 'No label')} "
