...
async def call_mcp_tool(request: Request) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(
            f"Error calling tool {tool_name if 'tool_name' in locals() else 'unknown'}: {e}"
        )
...
async def _run_tool(name: str, **kwargs: Any) -> Any:
    ...
    try:
        ...
        raise ValueError(f"Tool '{name}' not found or failed to execute: {e}")
...
async def api_processes(
    ...
    """List processes via MCP list_processes."""
    try:
        ...
        if result.get("status") != "success":
            return {"processes": [], "error": result.get("error", "unknown")}
...
async def api_process_detail(pid: int) -> dict[str, Any]:
    """Get process detail via analyze_process (or get_process_info)."""
    try:
        result = await _run_tool("analyze_process", pid=pid)
        if result.get("status") != "success":
...
async def api_services(
    ...
    """List Windows services via MCP list_services."""
    try:
        result = await _run_tool(
            "list_services",
        ...
        if result.get("status") != "success":
            return {"services": [], "error": result.get("error", "unknown")}
