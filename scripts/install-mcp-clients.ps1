...
foreach ($cfg in $configs) {
    ...
    try {
        ...
        if ($json.mcpServers."system-admin-mcp" -eq $null) {
            $json.mcpServers | Add-Member -Name "system-admin-mcp" -Value @{ url = "http://127.0.0.1:10861/mcp" } -MemberType NoteProperty
            $json | ConvertTo-Json -Depth 10 | Set-Content $cfg.Path
            Write-Host "  Registered system-admin-mcp in $($cfg.Name)" -ForegroundColor Green
            $updated++
        } else {
