...
import { Bot, Send, User, Sparkles, Brain, Zap, Settings2 } from "lucide-react";
...
export function Chat() {
    ...
    const chatHistory = [
        { id: 1, type: 'bot', text: "Systems online. Substrate monitoring active. How can I assist with your orchestration today?", time: '03:42:01' },
        { id: 2, type: 'user', text: "Check the status of the scanner service.", time: '03:42:15' },
        { id: 3, type: 'bot', text: "The Canon LiDE 400 bridge is currently idling. All ports are clear. CPU utilization for the bridge is < 1%.", time: '03:42:18' }
    ];
        ...
        <div className="flex flex-col h-[calc(100vh-120px)] space-y-4">
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4 p-2 bg-slate-900/50 border border-slate-800 rounded-xl">
                    ...
                    <Button size="icon" variant="ghost" className="h-8 w-8 text-slate-500 hover:text-blue-400 transition-colors">
            ...
            <Card className="flex-1 bg-slate-900/30 border-slate-800 backdrop-blur-xl flex flex-col overflow-hidden">
                <CardContent className="flex-1 overflow-auto p-6 space-y-6 scrollbar-thin scrollbar-thumb-slate-800">
                    {chatHistory.map((chat) => (
                        <div key={chat.id} className={`flex gap-4 ${chat.type === 'user' ? 'flex-row-reverse' : ''}`}>
                            <div className={`p-2 rounded-xl h-fit border shadow-lg ${chat.type === 'bot'
                                ? 'bg-blue-500/10 border-blue-500/20 text-blue-400'
                                : 'bg-slate-800/50 border-slate-700 text-slate-300'
                                }`}>
                                {chat.type === 'bot' ? <Sparkles className="w-4 h-4" /> : <User className="w-4 h-4" />}
                            </div>
                            <div className={`max-w-[80%] space-y-1 ${chat.type === 'user' ? 'text-right' : ''}`}>
                                <div className={`text-xs font-bold text-slate-500 uppercase tracking-tighter`}>
                                    {chat.type === 'bot' ? 'System Intelligence' : 'Sandra Schipal'} • {chat.time}
                                </div>
                                <div className={`p-4 rounded-3xl text-sm leading-relaxed ${chat.type === 'bot'
                ...
                <div className="p-6 border-t border-slate-800 bg-slate-950/50">
                    <div className="relative flex items-center gap-3">
                        <div className="p-2 rounded-lg bg-slate-950 border border-slate-800"><Brain className="w-5 h-5 text-purple-500" /></div>
                        <Input
                    ...
                    <div className="mt-3 flex items-center justify-between text-[10px] text-slate-500 px-2 uppercase font-bold tracking-widest">
                        <div className="flex items-center gap-3">
                            <span className="flex items-center gap-1"><Zap className="w-3 h-3 text-yellow-500" /> FastMCP Latency: 42ms</span>
                            <span className="flex items-center gap-1 text-slate-600 opacity-50">Context Tokens: 1.2M</span>
                        </div>
                        <div className="flex items-center gap-2">
