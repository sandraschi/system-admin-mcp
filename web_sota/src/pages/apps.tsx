    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-white">Apps Hub</h1>
                    <p className="text-slate-400 text-sm">Centralized SOTA fleet navigation</p>
                </div>
            </div>
                    ...
                    >
                        <Card className="h-full bg-slate-900/50 border-slate-800 backdrop-blur-xl transition-all duration-300 group-hover:bg-slate-800 group-hover:border-blue-500/50 group-hover:scale-[1.02] active:scale-95">
                            <CardHeader>
                                ...
                                <CardDescription className="text-slate-400 text-xs mt-1">{app.description}</CardDescription>
                            </CardHeader>
                            <CardContent>
                                <div className="flex flex-wrap gap-2">
                                    {app.tags.map(tag => (
                                        <Badge key={tag} variant="secondary" className="bg-slate-800 text-slate-400 text-[10px] capitalize">
                                            {tag}
                                        </Badge>
                                    ))}
            ...
            <div className="p-4 rounded-lg bg-orange-500/10 border border-orange-500/20 text-orange-400 text-xs">
                <strong>Discovery Protocol:</strong> Applications shown are registered in the SOTA master inventory. Links open in a new tab to preserve the admin session.
            </div>
        </div>
    );
}
